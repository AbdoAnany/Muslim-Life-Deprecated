...
class _GroupedListViewState<T, E> extends State<GroupedListView<T, E>> {
  ...
  Widget build(BuildContext context) {
    ...
    //     final Ayah value = _sortedElements[index] as Ayah;
    //     return Text(
      ...
      child: Stack(
        ...
        children: <Widget>[
          ...
          Wrap(
              ...
              children: List.generate(_sortedElements.length, (index) {
                {
                  var actualIndex = index;
                  if (actualIndex == 0) {
                    Ayah value = _sortedElements[actualIndex] as Ayah;
                    text = text +
                        "${value.text}(${value.numberInSurah.toString()})";
                    return Center(
                      child: Padding(
                  ...
                  if (actualIndex + 1 == _sortedElements.length) {
                    Ayah value = _sortedElements[actualIndex] as Ayah;
                    text = text +
                        "${value.text}"+ "(${value.numberInSurah.toString()})";
                    var x = text;
                  ...
                  if (actualIndex != 0) {
                    ...
                    if (prev != curr) {
                      final Ayah value = _sortedElements[actualIndex] as Ayah;
                      var x = text;
                      text = '';
                      text = text +
                          "${value.text}" +
                          "(${value.numberInSurah.toString()})";
                      return Column(
                        children: [
                          Text(
                            x,
                           // maxLines: 15,
                            ...
                            style: TextStyle(
                                //    fontSize: widget.fontSize
                               // fontSize: MediaQuery.of(context).size.width * .0456,
                         ...
                         //   overflow: TextOverflow.fade,
                          ...
                          _buildGroupSeparator(_sortedElements[actualIndex - 1]),
                        ],
                      );
                    }
                    final Ayah value = _sortedElements[actualIndex] as Ayah;
...
//       if (actualIndex == 0) {
//         Ayah value = _sortedElements[actualIndex] as Ayah;
//         text = text + "${value.text} (${value.numberInSurah.toString()})";
//         var x = text;
//
//         return   const Text(
//           '\nبِسْمِ ٱللَّٰهِ ٱلرَّحْمَٰنِ ٱلرَّحِيمِ',   style: TextStyle(
//           fontFamily: 'Kufi',
//           fontSize: 24,
//           color: Colors.black,
//         ),
...
//         return widget.separator;
//       }
//     }, childCount: _sortedElements.length));
//   }
//
